# Centrifuge 实时聊天室系统设计文档

## 1. 项目概述

基于 [Centrifugo](https://centrifugal.dev/) 构建的实时聊天室应用，支持：
- 多聊天室 (Channel) 管理
- 用户加入/离开聊天室的实时通知
- 实时消息发送和接收
- 在线用户列表 (Presence)
- 消息历史记录

## 2. 系统架构

```
┌─────────────────────────────────────────────────────────────────────────┐
│                              客户端 (Browser)                            │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │                    React Frontend Application                     │   │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────────────┐   │   │
│  │  │  Chat UI     │  │  User List   │  │  Channel Selector    │   │   │
│  │  └──────────────┘  └──────────────┘  └──────────────────────┘   │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                                    │                                     │
│                          WebSocket Connection                            │
│                       (centrifuge-js SDK)                               │
└────────────────────────────────────┼────────────────────────────────────┘
                                     │
                                     ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                         Centrifugo Server                                │
│                        (Port 8000 - WebSocket)                           │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │  Features:                                                        │   │
│  │  • Channel PUB/SUB          • Message History                    │   │
│  │  • Presence (online users)  • Join/Leave notifications           │   │
│  │  • JWT Authentication       • Proxy callbacks to backend         │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                                    │                                     │
│              Proxy Callbacks (HTTP POST)                                 │
│     ┌──────────┬──────────┬──────────┬──────────┐                       │
│     │ connect  │subscribe │ publish  │   RPC    │                       │
│     └────┬─────┴────┬─────┴────┬─────┴────┬─────┘                       │
└──────────┼──────────┼──────────┼──────────┼─────────────────────────────┘
           │          │          │          │
           ▼          ▼          ▼          ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                      Backend Server (Node.js)                            │
│                         (Port 3001 - HTTP)                               │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │  Proxy Handlers:                                                  │   │
│  │  • /centrifugo/connect    - 用户认证，返回用户信息               │   │
│  │  • /centrifugo/subscribe  - 验证订阅权限                         │   │
│  │  • /centrifugo/publish    - 验证并处理消息发布                   │   │
│  │  • /centrifugo/rpc        - 处理 RPC 调用                        │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │  REST API:                                                        │   │
│  │  • POST /api/auth/login   - 用户登录，返回 JWT                   │   │
│  │  • GET  /api/channels     - 获取可用频道列表                     │   │
│  │  • POST /api/channels     - 创建新频道                           │   │
│  │  • POST /api/messages     - 通过后端发送消息到 Centrifugo        │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                                    │                                     │
│                      Centrifugo Server API                               │
│                    (publish, presence, history)                          │
└────────────────────────────────────┼────────────────────────────────────┘
                                     │
                                     ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                          Database (Optional)                             │
│                     (SQLite / PostgreSQL / MongoDB)                      │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │  Tables:                                                          │   │
│  │  • users     - 用户信息                                          │   │
│  │  • channels  - 频道信息                                          │   │
│  │  • messages  - 消息持久化存储                                    │   │
│  └─────────────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────────────┘
```

## 3. 核心组件设计

### 3.1 Centrifugo 配置

```json
{
  "token_hmac_secret_key": "your-secret-key",
  "admin": true,
  "admin_password": "admin-password",
  "admin_secret": "admin-secret",
  "api_key": "your-api-key",
  "allowed_origins": ["http://localhost:3000"],

  "proxy_connect_endpoint": "http://localhost:3001/centrifugo/connect",
  "proxy_connect_timeout": "1s",

  "proxy_subscribe_endpoint": "http://localhost:3001/centrifugo/subscribe",
  "proxy_subscribe_timeout": "1s",

  "proxy_publish_endpoint": "http://localhost:3001/centrifugo/publish",
  "proxy_publish_timeout": "1s",

  "proxy_rpc_endpoint": "http://localhost:3001/centrifugo/rpc",
  "proxy_rpc_timeout": "1s",

  "namespaces": [
    {
      "name": "chat",
      "presence": true,
      "join_leave": true,
      "force_push_join_leave": true,
      "history_size": 100,
      "history_ttl": "300s",
      "force_recovery": true,
      "allow_subscribe_for_client": true,
      "allow_publish_for_subscriber": true,
      "allow_presence_for_subscriber": true,
      "allow_history_for_subscriber": true,
      "proxy_subscribe": true,
      "proxy_publish": true
    }
  ]
}
```

### 3.2 数据流程

#### 3.2.1 用户连接流程

```
1. 用户打开应用 → 前端加载
2. 用户登录 → 后端验证 → 返回 JWT Token
3. 前端使用 JWT 连接 Centrifugo
4. Centrifugo 调用 proxy_connect_endpoint 验证
5. 后端验证 JWT → 返回用户信息
6. 连接建立成功
```

#### 3.2.2 加入频道流程

```
1. 用户点击加入频道
2. 前端调用 subscription.subscribe()
3. Centrifugo 调用 proxy_subscribe_endpoint
4. 后端验证用户权限 → 返回允许/拒绝
5. 订阅成功 → Centrifugo 广播 join 事件
6. 频道内所有用户收到 join 通知
7. 前端更新在线用户列表
```

#### 3.2.3 发送消息流程

```
1. 用户输入消息 → 点击发送
2. 前端调用 subscription.publish(data)
3. Centrifugo 调用 proxy_publish_endpoint
4. 后端验证并处理消息:
   - 验证内容
   - 添加时间戳、消息ID
   - 可选：存储到数据库
   - 返回处理后的消息
5. Centrifugo 广播消息到频道
6. 所有订阅者收到消息
```

#### 3.2.4 离开频道流程

```
1. 用户点击离开 / 关闭页面
2. 前端调用 subscription.unsubscribe()
3. Centrifugo 检测到取消订阅
4. Centrifugo 广播 leave 事件
5. 频道内其他用户收到 leave 通知
6. 前端更新在线用户列表
```

## 4. 后端 API 设计

### 4.1 REST API

#### 用户认证
```
POST /api/auth/login
Request:  { "username": "string", "password": "string" }
Response: { "token": "jwt-token", "user": { "id": "string", "name": "string" } }

POST /api/auth/register
Request:  { "username": "string", "password": "string", "displayName": "string" }
Response: { "token": "jwt-token", "user": { "id": "string", "name": "string" } }
```

#### 频道管理
```
GET /api/channels
Response: { "channels": [{ "id": "string", "name": "string", "description": "string" }] }

POST /api/channels
Request:  { "name": "string", "description": "string" }
Response: { "channel": { "id": "string", "name": "string" } }

GET /api/channels/:id/messages
Response: { "messages": [{ "id": "string", "content": "string", "user": {...}, "timestamp": "string" }] }
```

### 4.2 Centrifugo Proxy Endpoints

#### Connect Proxy
```
POST /centrifugo/connect
Request (from Centrifugo):
{
  "client": "client-id",
  "transport": "websocket",
  "protocol": "json",
  "encoding": "json",
  "data": { "token": "jwt-token" }
}

Response:
{
  "result": {
    "user": "user-id",
    "expire_at": 1234567890,
    "info": { "name": "User Name", "avatar": "url" },
    "channels": ["chat:general"]  // 自动订阅的频道
  }
}
```

#### Subscribe Proxy
```
POST /centrifugo/subscribe
Request (from Centrifugo):
{
  "client": "client-id",
  "user": "user-id",
  "channel": "chat:room-1",
  "data": {}
}

Response:
{
  "result": {
    "info": { "name": "User Name" },
    "data": { "welcomeMessage": "Welcome to the room!" }
  }
}
// 或拒绝:
{
  "error": { "code": 403, "message": "Permission denied" }
}
```

#### Publish Proxy
```
POST /centrifugo/publish
Request (from Centrifugo):
{
  "client": "client-id",
  "user": "user-id",
  "channel": "chat:room-1",
  "data": { "text": "Hello!" }
}

Response:
{
  "result": {
    "data": {
      "id": "msg-uuid",
      "text": "Hello!",
      "user": { "id": "user-id", "name": "User Name" },
      "timestamp": "2024-01-15T10:30:00Z"
    }
  }
}
```

#### RPC Proxy
```
POST /centrifugo/rpc
Request (from Centrifugo):
{
  "client": "client-id",
  "user": "user-id",
  "method": "getOnlineUsers",
  "data": { "channel": "chat:room-1" }
}

Response:
{
  "result": {
    "data": {
      "users": [{ "id": "1", "name": "Alice" }, { "id": "2", "name": "Bob" }]
    }
  }
}
```

## 5. 前端组件设计

### 5.1 组件结构

```
src/
├── components/
│   ├── App.tsx                 # 主应用组件
│   ├── Auth/
│   │   ├── LoginForm.tsx       # 登录表单
│   │   └── RegisterForm.tsx    # 注册表单
│   ├── Chat/
│   │   ├── ChatRoom.tsx        # 聊天室主容器
│   │   ├── MessageList.tsx     # 消息列表
│   │   ├── MessageItem.tsx     # 单条消息
│   │   ├── MessageInput.tsx    # 消息输入框
│   │   └── TypingIndicator.tsx # 正在输入指示器
│   ├── Channel/
│   │   ├── ChannelList.tsx     # 频道列表
│   │   ├── ChannelItem.tsx     # 单个频道
│   │   └── CreateChannel.tsx   # 创建频道
│   └── User/
│       ├── UserList.tsx        # 在线用户列表
│       ├── UserItem.tsx        # 单个用户
│       └── UserProfile.tsx     # 用户资料
├── hooks/
│   ├── useCentrifuge.ts        # Centrifuge 连接 Hook
│   ├── useChannel.ts           # 频道订阅 Hook
│   ├── usePresence.ts          # 在线状态 Hook
│   └── useMessages.ts          # 消息管理 Hook
├── services/
│   ├── api.ts                  # REST API 客户端
│   └── centrifuge.ts           # Centrifuge 客户端封装
├── stores/
│   ├── authStore.ts            # 认证状态
│   ├── channelStore.ts         # 频道状态
│   └── messageStore.ts         # 消息状态
└── types/
    └── index.ts                # TypeScript 类型定义
```

### 5.2 核心 Hook 设计

#### useCentrifuge Hook

```typescript
interface UseCentrifugeOptions {
  url: string;
  token: string;
  onConnect?: () => void;
  onDisconnect?: (ctx: DisconnectedContext) => void;
}

function useCentrifuge(options: UseCentrifugeOptions) {
  const [client, setClient] = useState<Centrifuge | null>(null);
  const [connected, setConnected] = useState(false);

  // 初始化连接
  // 监听连接状态
  // 返回 client 实例和状态

  return { client, connected, disconnect };
}
```

#### useChannel Hook

```typescript
interface UseChannelOptions {
  client: Centrifuge;
  channel: string;
  onMessage?: (message: Message) => void;
  onJoin?: (info: JoinInfo) => void;
  onLeave?: (info: LeaveInfo) => void;
}

function useChannel(options: UseChannelOptions) {
  const [subscription, setSubscription] = useState<Subscription | null>(null);
  const [messages, setMessages] = useState<Message[]>([]);
  const [users, setUsers] = useState<User[]>([]);

  // 订阅频道
  // 监听消息、join/leave 事件
  // 提供发送消息方法

  return { subscription, messages, users, sendMessage, leave };
}
```

### 5.3 UI 布局

```
┌─────────────────────────────────────────────────────────────────────────┐
│  Header: Logo | Current User | Logout Button                            │
├───────────────────┬─────────────────────────────────┬───────────────────┤
│                   │                                 │                   │
│   Channel List    │         Chat Area               │   Online Users    │
│                   │                                 │                   │
│  ┌─────────────┐  │  ┌───────────────────────────┐  │  ┌─────────────┐  │
│  │ # general   │  │  │ Message 1                 │  │  │ 🟢 Alice    │  │
│  │ # random    │  │  │ Message 2                 │  │  │ 🟢 Bob      │  │
│  │ # tech      │  │  │ Message 3                 │  │  │ 🟢 Charlie  │  │
│  │             │  │  │ ...                       │  │  │             │  │
│  │ + New       │  │  │                           │  │  │             │  │
│  └─────────────┘  │  └───────────────────────────┘  │  └─────────────┘  │
│                   │                                 │                   │
│                   │  ┌───────────────────────────┐  │                   │
│                   │  │ Type a message... [Send]  │  │                   │
│                   │  └───────────────────────────┘  │                   │
│                   │                                 │                   │
└───────────────────┴─────────────────────────────────┴───────────────────┘
```

## 6. 项目结构

```
centrifuge-realtime-message-play/
├── docs/
│   └── DESIGN.md                    # 本设计文档
├── backend/                         # 后端服务
│   ├── package.json
│   ├── tsconfig.json
│   ├── src/
│   │   ├── index.ts                 # 入口文件
│   │   ├── config/
│   │   │   └── index.ts             # 配置管理
│   │   ├── routes/
│   │   │   ├── auth.ts              # 认证路由
│   │   │   ├── channels.ts          # 频道路由
│   │   │   └── centrifugo.ts        # Centrifugo proxy 路由
│   │   ├── services/
│   │   │   ├── auth.ts              # 认证服务
│   │   │   ├── channel.ts           # 频道服务
│   │   │   └── centrifugo.ts        # Centrifugo API 客户端
│   │   ├── middleware/
│   │   │   └── auth.ts              # JWT 验证中间件
│   │   └── types/
│   │       └── index.ts             # 类型定义
│   └── data/
│       └── db.json                  # 简单 JSON 数据存储
├── frontend/                        # 前端应用
│   ├── package.json
│   ├── tsconfig.json
│   ├── vite.config.ts
│   ├── index.html
│   └── src/
│       ├── main.tsx                 # 入口文件
│       ├── App.tsx                  # 根组件
│       ├── components/              # UI 组件
│       ├── hooks/                   # 自定义 Hooks
│       ├── services/                # API 服务
│       ├── stores/                  # 状态管理
│       ├── types/                   # 类型定义
│       └── styles/                  # 样式文件
├── centrifugo/                      # Centrifugo 配置
│   └── config.json                  # Centrifugo 配置文件
├── docker-compose.yml               # Docker 编排 (可选)
└── README.md                        # 项目说明
```

## 7. 技术栈

### 后端
- **运行时**: Node.js 20+
- **框架**: Express.js
- **语言**: TypeScript
- **JWT**: jsonwebtoken
- **HTTP Client**: axios (调用 Centrifugo API)
- **UUID**: uuid (生成消息ID)

### 前端
- **框架**: React 18+
- **构建工具**: Vite
- **语言**: TypeScript
- **状态管理**: Zustand
- **样式**: Tailwind CSS
- **Centrifuge SDK**: centrifuge (官方 JS 客户端)

### 实时服务
- **Centrifugo**: v5.x

## 8. 消息格式定义

### 聊天消息
```typescript
interface ChatMessage {
  id: string;              // UUID
  channel: string;         // 频道名称
  type: 'text' | 'image' | 'system';
  content: string;         // 消息内容
  user: {
    id: string;
    name: string;
    avatar?: string;
  };
  timestamp: string;       // ISO 8601 格式
  metadata?: Record<string, any>;
}
```

### Join/Leave 事件
```typescript
interface JoinLeaveEvent {
  type: 'join' | 'leave';
  user: {
    id: string;
    name: string;
  };
  channel: string;
  timestamp: string;
}
```

### Presence 数据
```typescript
interface PresenceInfo {
  client: string;          // Centrifugo client ID
  user: string;            // 用户 ID
  connInfo: {
    name: string;
    avatar?: string;
  };
  chanInfo?: Record<string, any>;
}
```

## 9. 安全考虑

1. **JWT 认证**: 所有 WebSocket 连接需要有效的 JWT Token
2. **频道权限**: 通过 subscribe proxy 验证用户加入频道的权限
3. **消息验证**: 通过 publish proxy 验证和清理消息内容
4. **CORS 配置**: 限制允许的来源域名
5. **Rate Limiting**: 后端实现消息发送频率限制
6. **XSS 防护**: 前端对消息内容进行转义处理

## 10. 部署架构

### 开发环境
```
前端开发服务器 (localhost:3000)
       │
       ▼
Centrifugo (localhost:8000)
       │
       ▼
后端服务 (localhost:3001)
```

### 生产环境
```
                    ┌─────────────────┐
                    │   Load Balancer │
                    │   (Nginx/ALB)   │
                    └────────┬────────┘
                             │
        ┌────────────────────┼────────────────────┐
        │                    │                    │
        ▼                    ▼                    ▼
┌───────────────┐   ┌───────────────┐   ┌───────────────┐
│  Frontend     │   │  Centrifugo   │   │   Backend     │
│  (CDN/Static) │   │  Cluster      │   │   Cluster     │
└───────────────┘   └───────────────┘   └───────────────┘
                             │
                             ▼
                    ┌───────────────┐
                    │     Redis     │
                    │  (PUB/SUB)    │
                    └───────────────┘
```

## 11. 下一步实施计划

1. **Phase 1**: 基础设施搭建
   - 初始化前端项目 (Vite + React + TypeScript)
   - 初始化后端项目 (Express + TypeScript)
   - 配置 Centrifugo

2. **Phase 2**: 核心功能实现
   - 用户认证系统
   - Centrifugo 连接管理
   - 基础聊天功能

3. **Phase 3**: 完善功能
   - 频道管理
   - 在线用户列表
   - 消息历史

4. **Phase 4**: 优化和完善
   - UI/UX 优化
   - 错误处理
   - 测试

---

## 参考资源

- [Centrifugo 官方文档](https://centrifugal.dev/)
- [Centrifugo GitHub](https://github.com/centrifugal/centrifugo)
- [Centrifuge-JS SDK](https://github.com/centrifugal/centrifuge-js)
- [Proxy Events 文档](https://centrifugal.dev/docs/server/proxy)
- [Channels 配置](https://centrifugal.dev/docs/server/channels)
